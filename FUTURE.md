...
- [x] JWT auth on write endpoints (`POST/PUT/DELETE /api/feeds`).
- [x] Clean architecture — `internal/{feed,user}/` with `model / repository / service / huma_handler`.
...
- [x] Domain error sentinels — `feed.ErrNotFound`, `user.ErrInvalidCredentials`, `auth.ErrInvalidToken`, `auth.ErrExpiredToken`. Handlers translate to HTTP status; never string-compare.
- [x] Semantic API Error Translators — `internal/platform/pgerr` translates `pgconn.PgError` codes + constraint names to domain sentinels (`user.ErrUsernameTaken`, `user.ErrRefreshTokenCollision`); `api.MapError` funnels every handler error through one call. Email is no longer a unique column, so `user.ErrEmailTaken` was removed.
...
- [x] Audit fields — `created_at`, `updated_at`, `deleted_at` (soft deletes) on `feeds` and `users`.
- [x] Integration testing — `testcontainers-go` Postgres in `backend/internal/feed/repository_integration_test.go`.
- [x] Connection pooling — pgxpool settings via viper.
- [x] Cache-aside — Redis for `GET /api/feeds` (opt-in via `REDIS_ENABLED=true`). Best-effort, never fails the request.
- [x] Index optimization — `migrations/000006_add_feeds_indexes.sql`, `000007_add_feeds_description_trgm_index.sql`.
...
- [x] Feature-based folder structure (`src/features/{feeds,auth}/`).
- [x] Server state — TanStack Query (caching, background refetch, pagination).
- [x] Form validation — React Hook Form + Zod.
- [x] Client state — Zustand (`authStore`, `uiStore`, `feedUiStore`).
...
- [x] Optimistic updates — `useFeeds.ts` uses `onMutate` for create / update / delete.
- [x] Loading skeletons — `SkeletonCard`, `FeedListSkeleton` in `FeedList.tsx`.
...
- [x] Multi-tab concurrent refresh race — `BroadcastChannel('nyx-tokens')` syncs `setTokens`/`clear` across tabs of the same origin; a freshly-opened tab whose `sessionStorage` is empty posts `{type:'request'}` and waits up to 200ms for a live peer to respond with the current pair before its first auth request fires. The reconciliation hook + wrapped-`api` request interceptor both `await tokenStore.whenReady()`. Same-origin only, transient messages — strictly safer than localStorage; equivalent XSS exposure to sessionStorage. `services/api.ts` + `services/tokenStore.test.ts`.
