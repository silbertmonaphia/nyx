...
- [x] **User huma_handler test coverage** — `user/huma_handler_test.go` covers register/login happy paths, validation rejections (missing/short username, short password), duplicate 409, unknown user 401, bad password 401, and 500 paths. Email is now optional, so the invalid-email and email-taken cases were removed.
